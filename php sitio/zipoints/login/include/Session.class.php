<?php
// $Id: session.php 321 2008-10-29 04:32:33Z KVron $
define('SESSIONS_DIR', dirname(__FILE__).'/_sessions_');
define('CLEAN_ALL_MINUTES', 60);
@mkdir(SESSIONS_DIR);
...
class Session {
    ...
    */ 
    public function __construct($login_required=false, $login_page = '/index.php', $domain_cookie = '', $session_dir = SESSIONS_DIR) {
        ...
        */
        if (!strlen(trim($session_dir))) {
            $session_dir = ini_get('session.save_path');
            if (!strlen($session_dir))
                $this->dir = realpath("./")."/"; //<-- NOT GOOD!
            else {
                if ($session_dir{strlen($session_dir-1)} != '/')
                    $session_dir.='/';
                @mkdir($session_dir);
                $this->log .= "\n\nCreating Session Dir: ".$session_dir."\n\n<br>\n\n";
            ...
            @mkdir($session_dir);
            if (!is_dir($session_dir))
                die('SESSION DIR DOES NOT EXIST: '.$session_dir);
            else
                $this->dir = $session_dir;
            $this->dir = $session_dir;
        }
    
        $this->log .= "Session dir: $this->dir\n\n<br>\n\n";
        ...
        if ($login_required) {
            ...
            if (!$this->data['logged_in']) {
                $this->log .= "Not logged in, redirecting to ".$this->login_page."<br />"; 
    ...
    function expire() {
        ...
        if (!file_exists($this->filename)) {
            ...
            if (!@unlink($this->filename)) {
                $this->log .= "session file delete failed for ".$this->filename."<br />";
                $ret = false;
            }
        }
        if (!setcookie('sid' ,$this->id, time()-3600, "/")) {
            $this->log .= "sid cookie expire failed. This may be due to browser output started prior.<br />";
            $ret = false;
        }
        return $ret;
    }
    ...
    */
    ...
    /*
    newId() generates a 40 character identifier that is extremely difficult to
